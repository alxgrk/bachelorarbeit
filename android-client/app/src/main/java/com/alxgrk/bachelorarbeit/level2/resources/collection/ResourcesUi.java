...
import com.alxgrk.bachelorarbeit.level2.resources.ResourceFragment;
...
class ResourcesUi {
    ...
    private void processSelfLink(Resource res, ConstraintLayout result) {
        ...
        entryLayout.setOnClickListener(v -> {
            sharedUi.getFragment().switchTo(ResourceFragment.newInstance(entryUrl + "resources/" + res.getId()));
        });
    }
    ...
    private void processAdminsLink(Resource res, ConstraintLayout result) {
        ...
        btnRes.setOnClickListener(v -> {
            getSharedUi().getFragment().switchTo(AccountsFragment.newInstance(entryUrl + "resources/" + res.getId() + "/administrators"));
        });
    }
